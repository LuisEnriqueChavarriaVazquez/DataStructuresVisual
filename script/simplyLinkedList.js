...
//append
let valueSimplyListAppend = myTab.shadowRoot.getElementById('valueSimplyListAppend');
valueSimplyListAppend.addEventListener('keydown', (event) => {
    if(event.key === 'Enter'){
        event.preventDefault();
        appendSimply(event.target.value, mySinglyLinkedList.getLength());
    }
});

function appendSimply(value, longitud){
    //Creamos el elemento para la cola
    mySinglyLinkedList.append(value);
    printSimplyLinkedList(longitud, tipo = "append");
}

//prepend
let valueSimplyListPrepend = myTab.shadowRoot.getElementById('valueSimplyListPrepend');
valueSimplyListPrepend.addEventListener('keydown', (event) => {
    if(event.key === 'Enter'){
        event.preventDefault();
        prependSimply(event.target.value,mySinglyLinkedList.getLength());
    }
});

function prependSimply(value, longitud){
    mySinglyLinkedList.prepend(value);
    printSimplyLinkedList(longitud, tipo = "prepend");
}

//printContent
//Lo que esta función hace es la impresión cada vez que actualizamos el contenido
function printSimplyLinkedList(elementosEnLista, tipo){
    let contenedorListasSimples = myTab.shadowRoot.getElementById('contenedorListasSimplesID');
    contenedorListasSimples.innerHTML = "";

    console.log(mySinglyLinkedList)

    if(elementosEnLista == 1){
            contenedorListasSimples.innerHTML += `
            <div class="nodoListaSimple">
                <div class="nodoListaDatos">
                    <span>Head</span>
                    <span>${mySinglyLinkedList.head.value}</span>
                    <span>Next</span>
                </div>
                <div class="nodoListaPointer">
                    <span></span>
                    <span>➡️</span>
                    <span></span>
                </div>
        ...
        `;
    }else if(elementosEnLista == 0){
        contenedorListasSimples.innerHTML = "Empty list"
    }else if(elementosEnLista > 1){
        contenedorListasSimples.innerHTML += `
            <div class="nodoListaSimple">
                <div class="nodoListaDatos">
                    <span>Head</span>
                    <span>${mySinglyLinkedList.search(0).next.value}</span>
                    <span>Next</span>
                </div>
                <div class="nodoListaPointer">
                    <span></span>
        ...
        for(let i = 1; i < elementosEnLista-1; i++){
            contenedorListasSimples.innerHTML += `
                <div class="nodoListaSimple">
                    <div class="nodoListaDatos">
                        <span></span>
                        <span>${mySinglyLinkedList.search(i).next.value}</span>
                        <span>Next</span>
                    </div>
                    <div class="nodoListaPointer">
                        <span></span>
        ...
        contenedorListasSimples.innerHTML += `
            <div class="nodoListaSimple">
                <div class="nodoListaDatos">
                    <span>Tail</span>
                    <span>${mySinglyLinkedList.tail.value}</span>
                    <span>Next</span>
                </div>
                <div class="nodoListaPointer">
                    <span></span>
